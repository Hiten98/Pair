  ...
  render() {
    ...
    if (/Android|webOS|iPhone|iPad|iPod|BlackBerry|BB|PlayBook|IEMobile|Windows Phone|Kindle|Silk|Opera Mini|Mobile/i.test(navigator.userAgent) || width < 450) {
      return (
        <Col xs={12} sm={6}>
          <hr/>
          <Row style={{ width: '90%', marginLeft: '5%' }}>Is it okay if your roommate has a pet?</Row>
          <Row style={{ width: '90%', marginLeft: '5%' }}>
            <DropDownMenu maxHeight={250} value={this.props.dv} onChange={this.props.themBringPetChange}>
              {items}
            </DropDownMenu>
          </Row>
        </Col>
      ...
      return (
        <Col xs={12} sm={6}>
          ...
          <Col xs={7} sm={8}>Is it okay if your roommate has a pet?</Col>
          <Col xs={5} sm={4}>
            <DropDownMenu maxHeight={250} value={this.props.dv} onChange={this.props.themBringPetChange}>
              {items}
            </DropDownMenu>
